...
public class Insert {
	public static void insert(String ID,String name,String lever,String programme) throws SQLException {
		Connection conn=new DataBaseConnection().getConnection();
		String sql="insert into Water(ID,name,lever,programme) values(?,?,?,?)";   //SQL语句
		PreparedStatement pstmt=conn.prepareStatement(sql);
        pstmt.setString(1,ID);
        pstmt.setString(2,name);
        pstmt.setString(3,lever);
        pstmt.setString(4,programme);
        pstmt.executeUpdate();
        String sql1="select * from Water";
        PreparedStatement pstmt1=conn.prepareStatement(sql1);
        ResultSet rs=pstmt1.executeQuery();
			JFrame f = new JFrame("增加后的水资源数据");
			f.setSize(500, 600);
